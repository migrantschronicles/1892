...
public class DiaryPages : MonoBehaviour
{
    ...
    private void Start()
    {
        foreach(DiaryEntryData entry in NewGameManager.Instance.DiaryEntries)
        {
            AddEntry(entry);
        }
    ...
    public GameObject CreatePageContent(DiaryEntryData entryData, DiaryPageData data, Transform parent, bool animated)
    {
        if(!data.IsValid)
        {
            return null;
        ...
        diaryPage.SetData(entryData, data);
    ...
    public void AddEntry(DiaryEntryData entry)
    {
        StopAnimators(false);
        if(lastAddedPage)
        {
